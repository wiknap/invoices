...
public class InvoiceDocument : IDocument
{
    ...
    private void ComposeHeader(IContainer container)
    {
        container.Text(text =>
        {
            ...
            text.Line($"{invoiceData.Seller.Name} - {invoiceData.Seller.GetFirstAddressLine()} - {invoiceData.Seller.GetSecondAddressLine()}");
        });
    }
    ...
    private void ComposeContent(IContainer container)
    {
        container.PaddingVertical(2, Unit.Centimetre)
            .Grid(grid =>
            {
                ...
                grid.Item().AlignRight().Text(text =>
                {
                    text.Line("Line");
                });
                grid.Item(2).AlignRight().Text(text =>
                {
                    text.Line("Line");
                });
    ...
    private void ComposeFooter(IContainer container)
    {
        container.Text(text =>
        {
            text.DefaultTextStyle(s => s.FontSize(6));
            text.Line("If undeliverable, please return to:");
            text.Line("Wiktor Napierała - Grochowska 291/13 - 03-842 Warszawa - Poland ");
        });
    }
}